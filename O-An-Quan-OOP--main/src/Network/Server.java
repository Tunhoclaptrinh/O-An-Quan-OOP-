...
//            int randomPort = 1024 + random.nextInt(64512);
            InetAddress serverAddress = InetAddress.getByName("0.0.0.0");  // Địa chỉ IP của máy chủ trong mạng LAN
            serverSocket = new ServerSocket(12369, 2, serverAddress); //backlog => giới hạn 2 client  truy cập vào
//            int randomport = serverSocket.getLocalPort();
            System.out.println("Server đang chờ người chơi...");
//            System.out.println("Server đang lắng nghe tại cổng: " + randomPort);
            ...
            String player2Name = reader2.readLine();  // Đọc tên người chơi từ client
    ...
    private static void printBoard() {
        ...
        for (int k = 10; k > 5; k--) {
            if (k < oDans.size()) { // Kiểm tra chỉ số hợp lệ
                board.append(ODan.sumDans(((ODan) oDans.get(k)).getDans())).append(" | ");
            } else {
                System.out.println("Chỉ số k = " + k + " vượt quá kích thước của oDans.");
            }
        }
